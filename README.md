...
[![Open T4 in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/khanhmay004/2A202600858-VoHuyenKhanhMay-Day22-Track3-DPO-Alignment-Lab/blob/main/colab/Lab22_DPO_T4.ipynb)
...
```bash
git clone https://github.com/khanhmay004/2A202600858-VoHuyenKhanhMay-Day22-Track3-DPO-Alignment-Lab.git
cd 2A202600858-VoHuyenKhanhMay-Day22-Track3-DPO-Alignment-Lab
...
1. **NB1** — `adapters/sft-mini/` written; `02-sft-loss.png` shows monotonic decrease.
...
5. **NB5 (OPTIONAL/bonus)** — `gguf/lab22-dpo-Q4_K_M.gguf` exists; `06-gguf-smoke.png` shows llama.cpp output.
...
| **Trainers** | TRL | ≥ 0.16, < 0.20 | `DPOTrainer` + `DPOConfig` (deck §5.2 surface) |
...
| Unsloth + TRL version mismatch | Pin: `unsloth>=2025.10 trl>=0.16,<0.20`. Nếu lỗi sau Unsloth update, downgrade Unsloth |
...
1. **Fork hoặc copy repo này lên GitHub account của bạn**, set repo **public**.
   ```bash
   git init -b main
   git remote add origin https://github.com/khanhmay004/2A202600858-VoHuyenKhanhMay-Day22-Track3-DPO-Alignment-Lab.git
   ```
